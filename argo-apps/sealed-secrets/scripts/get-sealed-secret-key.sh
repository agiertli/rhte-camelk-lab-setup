...
oc get secret -l sealedsecrets.bitnami.com/sealed-secrets-key=active -n tooling -o yaml >  ~/.bitnami/sealed-secrets-secret.yaml

echo "Get the public key from the Sealed Secrets secret."
NUM=$(oc get secret -l sealedsecrets.bitnami.com/sealed-secrets-key=active -n tooling -o name | wc -l)
if [ "${NUM}" -gt "1" ]; then
